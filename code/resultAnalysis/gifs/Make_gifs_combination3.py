...
x_span = [-18.44999999999999, 387.45]
y_span = [-18.44999999999999, 387.45]
z_span = [-84.94999999999999, 320.95]
...
def make_image(i):
    ...
    mask_im = Image.fromarray(np.uint8(cm.gist_stern_r(mask_s*51)*255)).convert('RGB')
    mask_im = cv2.addWeighted(np.rot90(np.array(im),k=1), 0.3,np.array(mask_im), 0.7, 0) / 255
