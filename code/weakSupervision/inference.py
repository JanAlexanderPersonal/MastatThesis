...
def get_model(model_list:List, model_location : str, modeltype_name : str):

    models = list()

    for i, model_dict in enumerate(model_list):
        model_folder = os.path.join(os.path.join(model_location), modeltype_name.replace('D', i), hu.hash_dict(model_dict))
        ...
        if os.path.exists(model_path):
            ...
            logging.warning('Model {model_folder} is not trained yet!')

        models.append(model)

    return models

def get_dataloaders(model_list : List, model_location):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m0', '--model_0',required=True, default=None)
    parser.add_argument('-m1', '--model_1', required=True, default=None)
    parser.add_argument('-m2', '--model_2', required=True, default=None)
    parser.add_argument('-mn', '--modellocation', required=True)
    parser.add_argument('-mn', '--modeltype_name', required=True)
    parser.add_argument('-d', '--datadir_name', default=None)
