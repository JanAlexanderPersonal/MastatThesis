...
def resampler(image: sitk.SimpleITK.Image, new_spacing: List[float] = None):
    ...
    """
    # Unless a specific spacing is given, the default spacing is 1 mm × 1 mm ×
    # 1 mm
    ...
    # Todo: Adapt this for non-label images -> not necessary to use nearest
    # neighbor
    ...
    new_size = np.array([x * (y / z)
                        for x, y, z in zip(orig_size, orig_spacing, new_spacing)])
