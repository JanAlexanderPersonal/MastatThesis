...
class FCN8(nn.Module):
    def __init__(self, n_classes):
        ...
        logger.info(f'Initialization of FCN RESNET50 with {n_classes}.')
    ...
    def forward(self, x):
        logging_output = [f'Input dimension: {x.size()}']
        self.resnet50_32s.eval()
        ...
        logging_output.append(f'After preparation layers: {x.size()}')

        x = self.resnet50_32s.layer1(x)

        logging_output.append(f'After layer 1 : {x.size()}')
        ...
        logging_output.append(f'Dimensions of the logit layers:')
        for l, ln in zip([logits_8s, logits_16s, logits_32s], ['logits 8', 'logits 16' , 'logits 32']):
            logging_output.append(f'dimension layer {ln} : {l.size()}')
        ...
        logging_output.append(f'Upsampled logits layer : {logits_upsampled.size()}')

        if LOGGING_F:
            for line in logging_output:
                logger.debug(line)
