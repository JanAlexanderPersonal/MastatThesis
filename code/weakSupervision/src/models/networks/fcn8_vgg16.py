...
class FCN8VGG16(nn.Module):
    def __init__(self, n_classes):
        ...
        logger.info(f'Initiation of FCN8 VGG16 with {n_classes} classes.')
    ...
    def forward(self, x):
        ...
        if LOGGING_F:
            logger.debug(f'Start forward step in FCN8 VGG16:')
            logger.debug(f'input dimension: {x.size()}')
            logger.debug(f'dimension before pool 1: {conv1_2.size()}')
            i = 1
            for p in [pool1, pool2, pool3, pool4, pool5]:
                logger.debug(f'dimension after pool {i}: {p.size()}')
                i += 1
            logger.debug(f'after fully connected 7 : {fc7.size()} --> result scoring layer: {scores.size()}')
            logger.debug(f'dimension score_pool4 : {score_pool4.size()} and upscore_pool4 : {upscore_pool4.size()}')
            logger.debug(f'dimension output : {output.size()} and return {output[:, :, 31: (31 + h), 31: (31 + w)].contiguous().size()}')
...
def conv3x3(in_planes : int, out_planes : int, stride: int=1, padding : int=1):
    ...
    Also see : https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html
