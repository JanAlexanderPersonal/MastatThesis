...
class Upsampler(nn.Module):
    def __init__(self, expansion_rate, n_output):
        # ToDo: expansion_rate is not used
        super().__init__()
    ...
    def upsample(self, x_input, x_8s, x_16s, x_32s):
        ...
        # size pyTorch tensor: [N, C, H, W]
        logits_16s_spatial_dim = logits_16s.size()[2:]
        logits_8s_spatial_dim = logits_8s.size()[2:]
        
        # torch.nn.functional.interpolate(input, size=None, scale_factor=None, mode='nearest', align_corners=None, recompute_scale_factor=None)
        # down or upsample the input to dimension 'size'

        # logits_upsampled is the sum of the scores upsampled up to the input spatial dimension
...
class FeatureExtracterStrong(nn.Module):
    def __init__(self):
        ...
        # change the final fully connected layer for 
    ...
    def extract_features(self, x_input):
        self.resnet50_32s.eval() # deactivate dropout
        x = self.resnet50_32s.conv1(x_input)
        x = self.resnet50_32s.bn1(x) # batch normalization
        x = self.resnet50_32s.relu(x)
        x = self.resnet50_32s.maxpool(x)

        # See code at https://chsasank.github.io/vision/_modules/torchvision/models/resnet.html 
...
class SamePad2dStrong(nn.Module):
    ...
    def forward(self, input):
        in_width = input.size()[2] # H
        in_height = input.size()[3] # W
